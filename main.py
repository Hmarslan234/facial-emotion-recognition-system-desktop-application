...
def contact():
    mess._show(title='Contact us', message="Please contact us on : 'contact@hmarslan.com' ")
...
#         faces = faceCascade.detectMultiScale(gray, 1.2, 5)
        ...
        for (x, y, w, h) in faces:
            ...
            cv2.putText(im, emotion_dict[maxindex], (x+20, y-60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
            if (conf < 50):
                ts = time.time()
                ...
                bb = str(aa)
                bb = bb[2:-2]
                attendance = [str(ID), '', bb, '', str(date), '', str(timeStamp), '', emotion]
    ...
    with open("Attendance\Attendance_" + date + ".csv", 'r') as csvFile1:
        ...
        for lines in reader1:
            ...
            if (i > 1):
                if (i % 2 != 0):
                    if len(lines) >= 9: # Check if there are enough values in the line
                        iidd = str(lines[0]) + '   '
...
window.title("SAHF - Sentiment Analysis of Human Face")
...
message3 = tk.Label(window, text="SAHF - Sentiment Analysis of Human Face" ,fg="white",bg="#262523" ,width=55 ,height=1,font=('times', 29, ' bold '))
...
tv= ttk.Treeview(frame1,height =13,columns = ('name','date','time','emotion'))
tv.column('#0',width=52)
tv.column('name',width=107)
tv.column('date',width=107)
tv.column('time',width=107)
tv.column('emotion',width=107)
